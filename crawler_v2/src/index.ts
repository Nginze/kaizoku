  ...
  try {
    ...
    for (let index = startingIdx; index < seedLimit; index++) {
      try {
        ...
        const animeMetaData = await anilistService.getAnilistMetaFromId(
          anilistId
        );
...
const generateSeedFileV2 = async () => {
  ...
  try {
    const mediaList = await anilistService.getBatchAnilistMetaFromIds(
      anilistIds
    );
