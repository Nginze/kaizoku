...
async function genMappings(animeMeta: any): Promise<any> {
  ...
  const url = `https://hianime.to/search/?keyword=${encodeURIComponent(bestTitle)}`;
  ...
  try {
    ...
    // Find best match
    let bestMatch = { index: 0, rating: 0 };
    searchResults.forEach((result, index) => {
      const rating = similarity(bestTitle.toLowerCase(), result.title.toLowerCase());
    ...
    if (bestMatch.rating > 0.3) {
      const matchedResult = searchResults[bestMatch.index];
    ...
    console.warn("No good match found for", bestTitle, "- best rating:", bestMatch.rating);
    return null;
  } catch (error: any) {
...
export async function processRequestedEmbeds(data?: any) {
  ...
  try {
    ...
    console.log(`[ProcessRequestedEmbeds] Found ${animeIds.length} anime(s) to process`);
    ...
    for (const anilistId of animeIds) {
      try {
        ...
        if (!anime) {
          console.warn(`[ProcessRequestedEmbeds] Anime ${anilistId} not found in database`);
        ...
        if (totalEpisodes === 0) {
          console.warn(`[ProcessRequestedEmbeds] Anime ${anilistId} has no episodes`);
          await redis.srem(REQUESTED_EMBEDS_KEY, anilistId);
          continue;
        }
        ...
        if (existingKeys.length > 0) {
          console.log(`[ProcessRequestedEmbeds] Anime ${anilistId} already has ${existingKeys.length} episodes with embeds, removing from queue`);
          await redis.srem(REQUESTED_EMBEDS_KEY, anilistId);
          continue;
        }
        ...
        if (!slug) {
          console.log(`[ProcessRequestedEmbeds] No cached mapping found, generating for ${anilistId}`);
          mappings = await genMappings(anime);
          slug = mappings?.aniwatch;
        }

        if (!slug) {
          console.warn(`[ProcessRequestedEmbeds] Could not find or generate aniwatch mapping for ${anilistId}`);
          await redis.srem(REQUESTED_EMBEDS_KEY, anilistId);
          continue;
        }
        ...
        for (let ep = 1; ep <= totalEpisodes; ep++) {
          try {
            console.log(`[ProcessRequestedEmbeds] Fetching episode ${ep}/${totalEpisodes} for anime ${anilistId}`);
            ...
            if (embeds.length > 0) {
              ...
              successCount++;
              console.log(`[ProcessRequestedEmbeds] Saved ${embeds.length} embeds for episode ${ep}`);
            } else {
              failedCount++;
              console.warn(`[ProcessRequestedEmbeds] No embeds found for episode ${ep}`);
            }

            await sleep(500);
          } catch (error: any) {
            failedCount++;
            console.error(`[ProcessRequestedEmbeds] Error processing episode ${ep}:`, error.message);
          }
        }
        ...
        console.log(`[ProcessRequestedEmbeds] Completed anime ${anilistId}: ${successCount} success, ${failedCount} failed`);

      } catch (error: any) {
        console.error(`[ProcessRequestedEmbeds] Error processing anime ${anilistId}:`, error.message);
        continue;
      }
    }
