...
export async function getMegaCloudClientKey(
  ...
  try {
    const req = await fetch(
      `https://megacloud.blog/embed-2/v3/e-1/${xrax}?k=1&autoPlay=0&oa=0&asi=1`,
      { headers: { Referer: "https://hianime.do/" } }
    );
    text = await req.text();
    ...
    if (count === 2) {
      ...
      clientKey = `${p1[0].replaceAll('"', "")}${p2[0].replaceAll(
        '"',
        ""
      )}${p3[0].replaceAll('"', "")}`;
    } else if (count === 1) {
...
export default async function extractToken(url: string): Promise<any> {
  try {
    const baseurl = "https://hianime.do";
